...
export default function App(): JSX.Element {
  ...
  async function toggleModel(id: string, next: boolean) {
    const nextIds = next ? [...new Set([...enabledIds, id])] : enabledIds.filter(x => x !== id);
    if (nextIds.length < 1) return; // keep >= 1
    try {
    ...
    <ToastProvider>
      <header className="app-header">
        <div className="app-header-inner">
          <div className="title">Deepfake Detect - React Frontend</div>
          ...
          <div className="stack">
            <section className="card stack">
              ...
              <div className="muted" style={{fontSize:12}}>
                API: {apiStatus === "ok" ? <span className="pill success">OK</span> : apiStatus === "loading" ? <span className="pill">Loading</span> : <span className="pill danger">Error</span>}
              </div>
            ...
            {models.length > 0 && (
              <ModelSelector models={models} onToggle={toggleModel} />
            )}
          </div>
