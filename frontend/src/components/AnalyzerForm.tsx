...
export default function AnalyzerForm({
  ...
  // Basic – Start/End (giây)
  const [startTime, setStartTime] = useState<string>("");
  const [endTime, setEndTime] = useState<string>("");

  // Advanced
  const [detectorBackend, setDetectorBackend] =
    useState<"retinaface" | "mediapipe">("retinaface");
  ...
  const numEnabled = enabledList.length;
  const isMultiModel = numEnabled >= 2;

  // Các model đang bật (có đủ name + id)
  const enabledModels: ModelMeta[] = (models ?? []).filter((m) =>
    enabledList.includes(m.id)
  );

  // XAI model selection (only among enabled models)
  const [xaiModelId, setXaiModelId] = useState<string>("auto");
  useEffect(() => {
    // Nếu lựa chọn hiện tại không còn trong danh sách model bật → reset về auto
    if (xaiModelId !== "auto" && !enabledList.includes(xaiModelId)) {
      setXaiModelId("auto");
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const D =
      fileInfo?.duration && Number.isFinite(fileInfo.duration)
        ? fileInfo.duration!
        : undefined;
    ...
    if (D !== undefined) {
      ...
      if (
        (startOut && (eVal === undefined || endOut)) ||
        (endOut && (sVal === undefined || startOut))
      ) {
        sVal = undefined;
        eVal = undefined;
      } else {
    ...
    // Chọn model dùng cho Grad-CAM (nếu bật)
    let selectedXaiModelId: string | undefined;
    if (xaiMode !== "none" && enabledList.length > 0) {
      if (xaiModelId === "auto" || !enabledList.includes(xaiModelId)) {
        // Auto = lấy model bật đầu tiên
        selectedXaiModelId = enabledList[0];
      } else {
        selectedXaiModelId = xaiModelId;
      }
    }
    ...
    const opts: AnalyzeOptions = {
      ...
      enabled_ids_csv:
        enabledIds && enabledIds.length ? enabledIds.join(",") : undefined,
      thr:
        !isMultiModel && thrOverride.trim() !== ""
          ? Number(thrOverride)
          : undefined,
      xai_mode: xaiMode,
      xai_model_id: selectedXaiModelId,
    };
    ...
    try {
      ...
      if (res?.thr_override_ignored) {
        addToast(
          "Multiple models enabled → using average threshold; override ignored. Warning"
        );
      } else {
    ...
    <form onSubmit={handleSubmit} className="stack">
          ...
          <div className="meta">
            {fileInfo ? (
              <>
                ...
                {fileInfo.duration ? (
                  <span>• {Math.round(fileInfo.duration)}s</span>
                ) : null}
              </>
            ) : (
      ...
      <div className="row">
        ...
        <div className="stack">
          ...
          {fileInfo?.duration ? (
            <div className="muted">
              Video duration: <b>{fileInfo.duration.toFixed(1)}s</b>
            </div>
          ) : null}
        </div>
      </div>
        ...
        <div className="stack">
          <div className="row">
            ...
            <div className="segmented" role="group" aria-label="Detector">
              <button
                type="button"
                aria-pressed={detectorBackend === "retinaface"}
                onClick={() => setDetectorBackend("retinaface")}
              >
                RetinaFace (for-GPU)
              </button>
              <button
                type="button"
                aria-pressed={detectorBackend === "mediapipe"}
                onClick={() => setDetectorBackend("mediapipe")}
              >
                MediaPipe (CPU-only)
              </button>
            </div>
          ...
          <div className="row">
            ...
            <input
              ...
              value={bboxScale}
              onChange={(e) =>
                setBboxScale(parseFloat(e.target.value || "1.10"))
              }
            />
          </div>
          ...
          {xaiMode === "full" && enabledModels.length >= 2 && (
            <div className="row">
              ...
              <div className="stack">
                <select
                  value={xaiModelId}
                  onChange={(e) => setXaiModelId(e.target.value)}
                >
                  <option value="auto">Auto (first enabled)</option>
                  {enabledModels.map((m) => (
                ...
                <div className="help">
                  Only models that are currently enabled can be selected for
                  Grad-CAM.
                </div>
              </div>
            </div>
          ...
          {isMultiModel && (
            <div className="muted">
              Multiple models are enabled. The backend will use the{" "}
              <b>average threshold</b> and ignore override.
            </div>
          )}
        </div>
      </details>
